...
namespace Firelands {
    ...
    public:
        ...
        void AddCreateObject(uint64 guid, TypeID typeId, MovementInfo const& move, std::map<uint16, uint32> const& fields) {
            ...
            if (flags & 0x80) { // LIVING
                uint8 guidBytes[8];
                for (int i = 0; i < 8; ++i) guidBytes[i] = (guid >> (i * 8)) & 0xFF;

                bool isZeroOrientation = (move.orientation == 0.0f);

                bw.WriteBit(true); // !Has MoveFlags0 (assuming move.flags == 0)
                bw.WriteBit(isZeroOrientation);
                bw.WriteBit(guidBytes[7] != 0);
                bw.WriteBit(guidBytes[3] != 0);
                bw.WriteBit(guidBytes[2] != 0);
                // skipped movementFlags since we wrote !Has = true
                
                bw.WriteBit(false); // !Has player spline data
                ...
                bw.WriteBit(true);  // !Has MoveFlags1 (assuming move.flags2 == 0)

                bw.Flush();

                _data.WriteByteSeq(guidBytes[4]);
                _data.Append<float>(4.5f); // RUN_BACK
                _data.Append<float>(2.5f); // SWIM_BACK
                _data.Append<float>(move.z);
                _data.WriteByteSeq(guidBytes[5]);
                _data.Append<float>(move.x);
                _data.Append<float>(3.14159f); // PITCH_RATE
                _data.WriteByteSeq(guidBytes[3]);
                _data.WriteByteSeq(guidBytes[0]);
                _data.Append<float>(4.722222f); // SWIM
                _data.Append<float>(move.y);
                _data.WriteByteSeq(guidBytes[7]);
                _data.WriteByteSeq(guidBytes[1]);
                _data.WriteByteSeq(guidBytes[2]);
                _data.Append<float>(2.5f); // WALK
                _data.Append<uint32>(move.time); // GameTime
                _data.Append<float>(3.14159f); // TURN_RATE
                _data.WriteByteSeq(guidBytes[6]);
                _data.Append<float>(7.0f); // FLIGHT
                if (!isZeroOrientation) _data.Append<float>(move.orientation);
                _data.Append<float>(7.0f); // RUN
                _data.Append<float>(4.5f); // FLIGHT_BACK
            } else {
                bw.Flush();
            }
