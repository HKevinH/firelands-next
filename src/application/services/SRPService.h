...
class SRPService {
public:
  static SRPData GenerateVerifier(const std::string &username,
                                  const std::string &password) {
    std::vector<uint8_t> salt(32);
    std::random_device rd;
    std::generate(salt.begin(), salt.end(),
                  [&]() { return static_cast<uint8_t>(rd() % 256); });
    ...
    std::reverse(
        x_bytes.begin(),
        x_bytes.end()); // To Little Endian for BigInt? No, BigInt expects BE.
  ...
  static BigInt GeneratePrivateB() {
    ...
    std::random_device rd;
    std::generate(b_bytes.begin(), b_bytes.end(),
                  [&]() { return static_cast<uint8_t>(rd() % 256); });
  ...
  static std::vector<uint8_t> CalculateSessionKey(const BigInt &A,
                                                  const BigInt &B,
                                                  const BigInt &v,
                                                  const BigInt &b) {
    BigInt bn_N(SRP::N);
  ...
  static std::vector<uint8_t> CalculateM1(const std::string &username,
    ...
    std::vector<uint8_t> data(buffer.GetBuffer(),
                              buffer.GetBuffer() + buffer.Size());
    return Crypto::CalculateSHA1(data);
  }

  static std::vector<uint8_t> CalculateM2(const BigInt &A,
