...
void CommandService::LoadCommandsFromDb() {
  ...
  handlers["mmap"]     = [this](auto s, auto a, auto o) { return _mmapCommands.Handle(s, a, o, _onlineCharacters.get()); };
...
void CommandService::PollScheduledRestart() {
  ...
  _mmapCommands.SweepExpiredMmapMarkers(_onlineCharacters.get());
  if (!_restartDeadline)
    return;
