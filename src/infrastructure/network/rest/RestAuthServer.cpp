...
namespace Firelands {
    ...
    RestAuthServer::RestAuthServer(std::shared_ptr<AuthService> authService, std::shared_ptr<WebSessionService> webSessionService, const std::string& address, uint16_t port)
        : _authService(std::move(authService)), _webSessionService(std::move(webSessionService)), _address(address), _port(port), _running(false) {
    }
    ...
    bool RestAuthServer::Start() {
        try {
            ...
            _acceptor = std::make_unique<tcp::acceptor>(_ioc, tcp::endpoint{address, _port});
            _running = true;
    ...
    void RestAuthServer::DoAccept() {
        _acceptor->async_accept([this](beast::error_code ec, tcp::socket socket) {
            if (!ec) {
                ...
                auto session = std::make_shared<std::thread>([this, socket = std::move(socket)]() mutable {
                    try {
    ...
    void RestAuthServer::HandleRequest(http::request<http::string_body>&& req, tcp::socket& socket) {
        ...
        if (req.method() == http::verb::post && req.target() == "/auth/login") {
            try {
                ...
                if (username.empty() || password.empty()) {
                    ...
                    if (valid) {
                        ...
                        res.body() = json({{"success", false}, {"error", "Invalid credentials"}}).dump();
        ...
        } else if (req.method() == http::verb::get && req.target() == "/auth/verify") {
            ...
            if (it == req.end()) {
                ...
                if (session) {
                    res.result(http::status::ok);
                    res.body() = json({{"valid", true}, {"accountId", session->accountId}}).dump();
                } else {
                    res.result(http::status::unauthorized);
                    res.body() = json({{"valid", false}, {"error", "Invalid or expired token"}}).dump();
                }
            }
        } else {
