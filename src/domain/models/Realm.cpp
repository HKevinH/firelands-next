...
Realm::Realm(uint32_t id, std::string name, std::string address, uint16_t port, uint8_t icon, uint8_t timezone, uint8_t allowedSecurityLevel, float population)
    : m_id(id), m_name(std::move(name)), m_address(std::move(address)), m_icon(icon), m_timezone(timezone), m_allowedSecurityLevel(allowedSecurityLevel) {
    if (port == 0) {
...
uint8_t Realm::GetAllowedSecurityLevel() const { return m_allowedSecurityLevel; }
